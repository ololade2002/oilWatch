    ...
    try:
        ...
        for record in event.get('Records', []):
            if record['eventName'] in ['INSERT', 'MODIFY']:
                ...
                asset_id = item.get("raw_asset_id") or item.get("asset_id") or "UNKNOWN_ASSET"
                ...
                if "wellhead_pressure_psi" in metrics:
                    ...
                    ceiling_value = 1500.0
                    if current_value > ceiling_value:
                        is_breached = True
                        severity = "Critical"
                        message = "Pressure breached safety ceiling — check downstream valve loops on Manifold Alpha."

                elif "manifold_pressure_psi" in metrics:
                    current_value = float(metrics["manifold_pressure_psi"])
                    ceiling_value = 1450.0  
                    if current_value > ceiling_value:
                        is_breached = True
                        severity = "Critical"
                        message = "Manifold pressure exceeds gathering system limits — adjust pressure control regulator."

                elif "vessel_pressure_psi" in metrics:
                    current_value = float(metrics["vessel_pressure_psi"])
                    ceiling_value = 1400.0  
                    if current_value > ceiling_value:
                        is_breached = True
                        severity = "Critical"
                        message = "Separator pressure approaching high operating limit — verify gas flare line valve alignment."

                if "oil_level_percentage" in metrics and not is_breached:
                ...
                if is_breached:
                    ...
                    try:
                        ...
                        if response.get('Items'):
                            ...
                            print(f"Active alert already exists for {asset_id}. Suppressing duplicate email notification.")
                    except Exception as scan_err:
                        print(f"Could not check active alert states: {str(scan_err)}")

                    authorizer_context = event.get('requestContext', {}).get('authorizer', {})
                    user_email = authorizer_context.get('claims', {}).get('email', 'shift_supervisor@oilwatch.com')
                    ...
                    try:
                        ...
                        print(f"Non-fatal database write failure: {str(db_err)}")
                    ...
                    if SNS_TOPIC_ARN and not already_notified:
                        try:
                            ...
                            print(f"First-occurrence notification routed successfully to operator.")
                        except Exception as sns_err:
                            print(f"Non-fatal SNS dispatch failure: {str(sns_err)}")
                    elif not SNS_TOPIC_ARN:
                        print("Notification skipped: ALERT_TOPIC_SNS ARN missing.")
                else:
                    print(f"Telemetry for {asset_id} is within stable operational tolerances.")
